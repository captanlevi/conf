...
from ..core.flowRepresentation import PacketFlowRepressentation, TimeslotRepresentation, MediaStreamRepresentation
...
def getFlowReps(dataset_name, max_flow_length):
    ...
    if dataset_name == "dummy":
        packet_flow_reps = loadFlows(path= "data/dummy/dummy.json", cls= MediaStreamRepresentation)
...
def readTrainTestOODFlows(base_path, dataset_name):
    ...
    try:
        train_packet_flows = loadFlows(path= os.path.join(dir_path,"train_packet_flows.pkl"), cls= MediaStreamRepresentation)
        test_packet_flows = loadFlows(path= os.path.join(dir_path,"test_packet_flows.pkl"), cls= MediaStreamRepresentation)
    except:
    ...
    if os.path.exists(os.path.join(dir_path,"ood_packet_flows.pkl")):
        try:
            ood_packet_flows = loadFlows(path= os.path.join(dir_path,"ood_packet_flows.pkl"), cls= MediaStreamRepresentation)
        except:
